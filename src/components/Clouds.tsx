...
import { useSceneDimensions } from '@components/ParallaxPage';
...
export default function Clouds({
  ...
  useEffect(() => {
    evolutionTimerRef.current = setInterval(() => {
      ...
      // Update targets for existing clouds
  ...
  useEffect(() => {
    if (sceneWidth === 0) return;
    // Tick function updates cloud positions and properties, spawns/despawns as needed, and schedules the next tick.
    const interval = 1000 / FRAME_RATE;
    function tick() {
      const now = Date.now();
      const elapsed = (now - lastFrameRef.current) / 1000; // seconds
      lastFrameRef.current = now;
      // Update cloud positions and properties based on speed and interpolation toward targets
  ...
  // Precompute cloud color with base RGB and per-puff opacity
  const [r, g, b] = cloudBaseRGB;
  // Render each cloud as a group of blurred ellipses (puffs)
  return (
    <div
      style={{
        position: 'absolute',
